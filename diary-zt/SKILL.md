...
保存可见用户消息和助手消息；工具结果仅保留简短结论。绝不保存系统提示词、开发者指令、密钥或其他隐藏上下文。
...
新文件包含标题、首次保存时间和 `## 会话记录 YYYY-MM-DD HH:MM` 小节。用户和助手消息按顺序保留原文。

工具结果默认压缩：

- 每个有意义的工具操作最多写一句“做了什么，结果如何”；
- 保留影响后续决策的结论、关键错误和最终验证结果；
- 省略原始命令输出、完整日志、文件全文回显、空轮询、进度等待和重复结果；
- 工具结果没有增加任何有用信息时，不写“工具结果”条目。

`/diary-zt-a` 只在目标文件末尾添加新小节；没有新增内容时报告“没有新增内容”且不修改文件。
